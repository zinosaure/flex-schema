...
class FlexmodelLite(Flex):
    ...
    @classmethod
    def attach(cls, database: Union[str, sqlite3.Connection], table_name: Optional[str] = None):
    ...
    @classmethod
    def table(cls) -> sqlite3.Connection:
        if not cls.connection:
            raise FlexmodelException("Database is not attached. Call attach() first.")
    ...
    def _execute(cls, query: str, params: tuple[Any, ...] = ()) -> sqlite3.Cursor:
        cursor = cls.table().execute(query, params)
        cls.table().commit()
        return cursor
    ...
    def commit(self, commit_all: bool = True) -> bool:
        ...
        self._execute(
            f"""
            INSERT INTO {self.table_name} (_id, _updated_at, document)
            VALUES (?, ?, ?)
    ...
    def delete(self) -> bool:
        cursor = self._execute(f"DELETE FROM {self.table_name} WHERE _id = ?", (self.id,))
        return cursor.rowcount > 0

    @classmethod
    def load(cls, id: str) -> Optional["FlexmodelLite"]:
        cursor = cls._execute(f"SELECT _id, _updated_at, document FROM {cls.table_name} WHERE _id = ?", (id,))
    ...
    @classmethod
    def count(cls) -> int:
        cursor = cls._execute(f"SELECT COUNT(*) AS count FROM {cls.table_name}")

        return int(item["count"]) if (item := cursor.fetchone()) else 0
    ...
    class Select:
        ...
        @property
        def to_sql(self) -> str:
            sql: str = "SELECT * FROM " + self.model.table_name
